...
% Empirically given parameter values
opts.tol = 1e-7;
opts.max_iter = 120;
opts.max_mu = 1e7;
opts.DEBUG = 1;
...
a = optimizableVariable('a', [0 0.01], 'Type', 'real');
b = optimizableVariable('b', [1e-8 1e-5], 'Type', 'real');
rho = optimizableVariable('rho', [1.1 1.2], 'Type', 'real');
mu = optimizableVariable('mu', [1e-7 1e-5], 'Type', 'real');

parameter = [a, b, rho, mu];
cd D:\brain_image\Code_ding\Matlab\CDLRR;
...
for iter = 1:max_iter
    ...
    % Pin s.t. Pin'Pin=I Orithogonal constraint
